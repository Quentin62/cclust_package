...
def get_parsers():
    ...
    subparsers = main_parser.add_subparsers(help="choose the algorithm to use", dest="subparser_name")

    coclust_nb_parser = argparse.ArgumentParser(prog="coclust-nb")

    # create the parser for the "modularity" command
    parser_modularity = subparsers.add_parser(
        "modularity",
    ...
    parser_spec_modularity = subparsers.add_parser(
        "specmodularity",
    ...
    parser_list = [parser_modularity, parser_spec_modularity, parser_info, coclust_nb_parser]
    for parser in parser_list:
        ...
        input_params_group.add_argument(
            "-k",
            "--matlab_matrix_key",
            default=None,
            help="if not set, csv \
        ...
        if parser == coclust_nb_parser:
            parameters_group.add_argument("--from", default=2, type=int, help="minimum number of co-clusters")
            parameters_group.add_argument("--to", default=10, type=int, help="maximum number of co-clusters")
        elif parser == parser_info:
            parameters_group.add_argument("-K", "--n_row_clusters", help="number of row clusters", default=2, type=int)
            parameters_group.add_argument("-L", "--n_col_clusters", help="number of column clusters", default=2, type=int)
        else:
    ...
    return {"coclust": main_parser, "coclust-nb": coclust_nb_parser}
...
def main_coclust_nb():
    ...
    for n_coclusters in range(args["from"], args["to"] + 1):
        model = CoclustMod(
            n_clusters=n_coclusters, max_iter=args["max_iter"], n_init=args["n_runs"], random_state=args["seed"]
        )
        model.fit(X)
    ...
    print("The best number of co-clusters is", best_model.n_clusters, "with a modularity of", best_model.modularity)
...
def process_visualization(args, model, X):
    if args["visu"]:
        try:
            ...
            plt.spy(X_reorg, precision=0.8, markersize=0.9, aspect="auto")
...
def process_evaluation(args, model):
    if args["true_row_labels"]:
        try:
            ...
            logging.error(
                "--true_row_labels option (evaluation) exception:\
                          %s"
                % e
            )
...
def info(args):
    ...
    model = CoclustInfo(
        ...
        n_col_clusters=args["n_col_clusters"],
        init=W,
        max_iter=args["max_iter"],
        n_init=args["n_runs"],
        tol=args["epsilon"],
